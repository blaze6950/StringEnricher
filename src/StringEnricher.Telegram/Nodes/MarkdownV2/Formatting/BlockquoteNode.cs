    ...
    where TInner : INode
{
        ...
        IFormatProvider? provider)
    {
        try
        {
            charsWritten = 1; // For the initial line prefix
            destination[0] = LinePrefix; // Write the line prefix

            charsWritten += BufferUtils.StreamBuffer(
                source: _innerText,
                destination: destination[1..], // Offset by 1 to account for the initial line prefix
                streamWriter: static (c, _, destination) =>
                {
                    if (!IsLineSeparator(c))
                    {
    ...
    private ValueTuple<int, int, int> CalculateLength(TInner innerText)
    {
        ...
        // plus one for the first line prefix
        var syntaxLength = result.EscapedCount + 1;
