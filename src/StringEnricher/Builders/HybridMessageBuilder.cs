...
/// <summary>
/// A builder for creating messages with a hybrid approach,
/// allowing efficient construction of messages with known total length.
/// If the total length is not known - this builder will find the needed length buffer internally and allocate accordingly.
///
/// It differs from <see cref="AutoMessageBuilder"/> in that it allows specifying an initial capacity hint for buffer allocation purposes
/// and this will allow to build a final string using a single buffer allocation (in stack, pool, heap)
/// that is configurable using <see cref="StringEnricherSettings"/> and then final string allocation.
/// Also, if the total length is not known, it will allocate a buffer continuously until the message is built successfully,
/// while <see cref="AutoMessageBuilder"/> can calculate the accurate needed length using a preliminary pass.
///
/// It differs from <see cref="MessageBuilder"/> in that it allows specifying an initial capacity hint for buffer allocation purposes and this hint can be bigger than the real final length,
/// allowing to avoid buffer reallocations in scenarios where the final length is not known upfront.
/// </summary>
public readonly struct HybridMessageBuilder
{
    private readonly int? _initialCapacityHint;

    /// <summary>
    /// Initializes a new instance of the <see cref="HybridMessageBuilder"/> struct.
    /// </summary>
    /// <param name="initialCapacityHint">
    /// An optional hint for the initial capacity of the buffer to allocate.
    /// If provided, the builder will attempt to allocate a buffer with at least this capacity.
    /// This can help reduce the number of allocations if the expected message size is known in advance but not exact.
    /// </param>
    public HybridMessageBuilder(int? initialCapacityHint)
    {
        _initialCapacityHint = initialCapacityHint;
    }

    /// <summary>
    /// Creates a message string using the specified state and build action.
    /// </summary>
    /// <param name="state">
    /// The state to be used in the build action.
    /// </param>
    /// <param name="buildAction">
    /// The action that builds the message using the provided state and a <see cref="MessageWriter"/>.
    /// </param>
    /// <typeparam name="TState">
    /// The type of the state to be used in the build action.
    /// </typeparam>
    /// <returns>
    /// The created message string.
    /// </returns>
    public string Create<TState>(
        TState state,
        Func<TState, MessageWriter, int> buildAction
    ) => BufferUtils.AllocateBuffer<StringMaterializer<TState>, TState, string>(
        processor: new StringMaterializer<TState>(buildAction),
        state: state,
        nodeSettings: (NodeSettingsInternal)StringEnricherSettings.Builders.HybridMessageBuilder,
        initialBufferLengthHint: _initialCapacityHint
    );
    ...
    public ref struct MessageWriter
    {
        ...
        /// <summary>
        /// Gets the total length of the appended parts.
        /// </summary>
        public int Length
        {
