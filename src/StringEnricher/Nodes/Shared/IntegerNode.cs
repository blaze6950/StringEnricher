...
public struct IntegerNode : INode
{
    ...
    public string ToString(string? format, IFormatProvider? provider)
    {
        ...
        var length = _integer.GetSpanFormattableLength(
        ...
        return string.Create(
            length: length,
            state: ValueTuple.Create(_integer, format, provider),
    ...
    public bool TryGetChar(int index, out char character)
    {
        ...
        {
#if UNIT_TESTS
            DebugCounters.IntegerNode_TryGetChar_CachedTotalLengthEvaluation++;
#endif
            character = '\0';
            return false;
        }
