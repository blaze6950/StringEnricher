...
public struct DateOnlyNode : INode
{
    ...
    public string ToString(string? format, IFormatProvider? provider)
    {
        ...
        var length = _dateOnly.GetSpanFormattableLength(
        ...
        return string.Create(
            length: length,
            state: ValueTuple.Create(_dateOnly, format, provider),
    ...
    public bool TryGetChar(int index, out char character)
    {
        ...
        {
#if UNIT_TESTS
            DebugCounters.DateOnlyNode_TryGetChar_CachedTotalLengthEvaluation++;
#endif
            character = '\0';
            return false;
        }
