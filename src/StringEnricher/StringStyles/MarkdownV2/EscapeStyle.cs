...
public static class EscapeMarkdownV2
{
    ...
    /// </param>
    /// <returns>
    /// A new instance of <see cref="EscapeStyle{PlainTextStyle}"/> wrapping the provided text.
    /// </returns>
    public static EscapeStyle<PlainTextStyle> Apply(string text) =>
        EscapeStyle<PlainTextStyle>.Apply(text);
    ...
    /// </typeparam>
    /// <returns>
    /// A new instance of <see cref="EscapeStyle{T}"/> wrapping the provided inner style.
    /// </returns>
    public static EscapeStyle<T> Apply<T>(T style) where T : IStyle =>
        EscapeStyle<T>.Apply(style);
}
...
/// </typeparam>
public readonly struct EscapeStyle<TInner> : IStyle
    where TInner : IStyle
{
    private const char
        EscapeSymbol =
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="EscapeStyle{TInner}"/> struct.
    /// </summary>
    /// <param name="inner">The inner style that will be escaped.</param>
    public EscapeStyle(TInner inner)
    {
        _innerText = inner;
    ...
    /// </param>
    /// <returns>
    /// A new instance of <see cref="EscapeStyle{TInner}"/> wrapping the provided inner style.
    /// </returns>
    public static EscapeStyle<TInner> Apply(TInner innerStyle) => new(innerStyle);
    ...
    {
        private readonly EscapeStyle<TInner> _escapeStyle;
        ...
        /// </summary>
        /// <param name="escapeStyle">The escape style to iterate through.</param>
        public CharacterIterator(EscapeStyle<TInner> escapeStyle)
        {
            _escapeStyle = escapeStyle;
        ...
        public bool MoveNext(out char character)
        {
            ...
            {
                _escapeStyle._innerText.TryGetChar(_currentOriginalIndex, out character); // set character to the actual character to be escaped on a previous call
            ...
            if (_escapeStyle._innerText.TryGetChar(_currentOriginalIndex, out character))
            {
                _currentVirtualIndex++;
