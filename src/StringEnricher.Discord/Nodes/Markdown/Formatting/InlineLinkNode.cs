...
/// </typeparam>
[DebuggerDisplay(
    "{typeof(InlineLinkNode).Name,nq} Prefix={Prefix} InnerType={typeof(TInner).Name,nq} LinkSeparator={LinkSeparator} Suffix={Suffix}")]
    ...
    where TInner : INode
{
    ...
    )
    {
        ...
        // Copy link separator
        if (!LinkSeparator.AsSpan().TryCopyTo(destination.Slice(charsWritten, LinkSeparator.Length)))
        {
            return false;
        }
        ...
        // Copy link URL
        if (!_linkUrl.AsSpan().TryCopyTo(destination.Slice(charsWritten, _linkUrl.Length)))
        {
            return false;
        }
